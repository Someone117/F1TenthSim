...
namespace Infinite {
    ...
    void TexturedImage::create() {
        ...
        transitionImageLayout(this, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED,
        ...
        _image_view = createImageView(_image.image, VK_FORMAT_R8G8B8A8_SRGB, mipLevels);
        createSampler();
    }
    ...
    void TexturedImage::generateMipmaps(Image *image, VkFormat format, int width, int height, uint32_t mipLevels) {
        ...
        for (uint32_t i = 1; i < mipLevels; i++) {
            ...
            vkCmdBlitImage(commandBuffer,
                           image->getImage().image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                           image->getImage().image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                           1, &blit,
                           VK_FILTER_LINEAR);
    ...
    void TexturedImage::createSampler() {
        ...
        if (vkCreateSampler(device, &samplerInfo, nullptr, &_sampler) != VK_SUCCESS) {
            throw std::runtime_error("failed to createExtras texture sampler!");
        }
    ...
    VkSampler *TexturedImage::getSampler() {
        return &_sampler;
    }
