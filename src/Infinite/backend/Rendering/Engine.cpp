...
//        }
//
//        vkDestroyDescriptorPool(Engine::getEngine().getInstance(), descriptorPool, nullptr);
//
//        vkDestroyDescriptorSetLayout(Engine::getEngine().getInstance(), descriptorSetLayout, nullptr);
//
//
...
//
//        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
//            vkDestroySemaphore(Engine::getEngine().getInstance(), renderFinishedSemaphores[i], nullptr);
//            vkDestroySemaphore(Engine::getEngine().getInstance(), imageAvailableSemaphores[i], nullptr);
//            vkDestroyFence(Engine::getEngine().getInstance(), inFlightFences[i], nullptr);
//        }
//
...
// all done
    ...
    void Engine::createInstance() {
        ...
        if (vkCreateInstance(&createInfo, nullptr, &instance) != VK_SUCCESS) {
            throw std::runtime_error("failed to createExtras instance!");
        }
    }
    ...
    void Engine::cleanupSwapChain() {
        for (const std::vector<VkFramebuffer> &swapChainFramebufferVector: swapChainFramebuffers) {
    ...
    void Engine::createSwapChain() {
        ...
        if (vkCreateSwapchainKHR(device, &createInfo, nullptr, &swapChain) != VK_SUCCESS) {
            throw std::runtime_error("failed to createExtras swap chain!");
        }
    ...
    SwapChainSupportDetails Engine::querySwapChainSupport(VkPhysicalDevice device) {
        ...
        vkGetPhysicalDeviceSurfaceCapabilitiesKHR(device, Engine::getEngine().surface, &details.capabilities);

        uint32_t formatCount;
        vkGetPhysicalDeviceSurfaceFormatsKHR(device, Engine::getEngine().surface, &formatCount, nullptr);

        if (formatCount != 0) {
            details.formats.resize(formatCount);
            vkGetPhysicalDeviceSurfaceFormatsKHR(device, Engine::getEngine().surface, &formatCount,
                                                 details.formats.data());
        }

        uint32_t presentModeCount;
        vkGetPhysicalDeviceSurfacePresentModesKHR(device, Engine::getEngine().surface, &presentModeCount,
                                                  nullptr);
    ...
    void Engine::createFramebuffers(RenderPass &renderPass, uint32_t renderPasses) {
        ...
        for (size_t i = 0; i < swapChainImageViews.size(); i++) {
            ...
            if (vkCreateFramebuffer(device, &framebufferInfo, nullptr,
                                    &swapChainFramebuffers[renderPass.getIndex()][i]) != VK_SUCCESS) {
                throw std::runtime_error("failed to createExtras framebuffer!");
            }
        }
    }
    ...
    VkResult
    Engine::CreateDebugUtilsMessengerEXT(VkInstance instance, const VkDebugUtilsMessengerCreateInfoEXT *pCreateInfo,
                                         const VkAllocationCallbacks *pAllocator,
    ...
    void Engine::endSingleTimeCommands(VkCommandBuffer commandBuffer, VkCommandPool commandPool) {
        ...
        vkQueueSubmit(Engine::getEngine().graphicsQueue, 1, &submitInfo, VK_NULL_HANDLE);
    ...
    VkQueue_T *Engine::getGraphicsQueue() {
    ...
    bool Engine::isFramebufferResized() const {
