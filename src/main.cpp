...
const char *const MODEL_PATH = R"(C:\VulkanProject\assets\viking_room.obj)";
const char *const TEXTURE_PATH = R"(C:\VulkanProject\assets\viking_room.png)";
...
void mainLoop() {
    ...
    if (glfwRawMouseMotionSupported()) // make a setting
        glfwSetInputMode(Engine::getEngine().getWindow(), GLFW_RAW_MOUSE_MOTION, GLFW_TRUE);
    ...
    while (!glfwWindowShouldClose(Engine::getEngine().getWindow())) {
        ...
        if (capture_cursor) {
            ...
            glfwSetCursorPos(Engine::getEngine().getWindow(), Engine::getEngine().getWindowWidth() / 2.0,
                             Engine::getEngine().getWindowHeight() / 2.0);
            // looking
            camera.mouse((std::floor(xpos - Engine::getEngine().getWindowWidth() / 2.0f) * spf),
                         (std::floor((ypos - Engine::getEngine().getWindowHeight() / 2.0f)) * spf));
...
int main() {
    ...
    Model mainModel("main", MODEL_PATH, TEXTURE_PATH);

    mainPass.addModel(&mainModel); // works
